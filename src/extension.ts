...
export function activate(context: vscode.ExtensionContext) {
  console.log('Congratulations, your extension "ggsgornil" is now active!');
  // The command has been defined in the package.json file
  // Now provide the implementation of the command with registerCommand
  // The commandId parameter must match the command field in package.json
  let disposable = vscode.commands.registerCommand('ggsgornil.generateGettersAndSetters', () => {
    // The code you place here will be executed every time your command is executed
    // Display a message box to the user
    const editor = vscode.window.activeTextEditor;
    let structName = '';
    const getters: string[] = [];
    const gettersNillable: string[] = [];
      ...
      for (let line = 0; line < lines.length; line++) {
        ...
        if (line === 0) {
          ...
          if (matches) {
            ...
            vscode.window.showErrorMessage(`Invalid selection. First line must be a Go struct definition.`);
            return;
          }
          continue;
        }
    ...
    if (getters.length + gettersNillable.length + setters.length >= 1) {
      vscode.window.showInformationMessage(`Generated ${getters.length} getters and ${setters.length} setters.`);
    }
  });
